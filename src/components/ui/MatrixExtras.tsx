...
// ============================================
// MatrixTimePicker - Custom scrollable time picker
// ============================================

interface MatrixTimePickerProps {
  label?: string;
  value: string;
  onChange: (value: string) => void;
  className?: string;
...
export function MatrixTimePicker({
  label,
  value,
  onChange,
  className = "",
}: MatrixTimePickerProps) {
  const [open, setOpen] = useState(false);
  const triggerRef = useRef<HTMLButtonElement>(null);
  const [pos, setPos] = useState({ top: 0, left: 0, width: 0 });
      ...
      {open &&
        createPortal(
          <>
            <div className="fixed inset-0 z-[9998]" onClick={() => setOpen(false)} />
            <div
              className="fixed z-[9999] matrix-panel border border-matrix-primary/30 shadow-[0_0_20px_rgba(0,255,65,0.1)] flex h-52"
              style={{ top: pos.top, left: pos.left, width: pos.width }}
            >
              <div className="flex-1 overflow-y-auto border-r border-matrix-primary/20 scrollbar-none">
                {hours.map((h) => (
                  <button
                    key={`h-${h}`}
                    type="button"
                    onClick={() => onChange(`${h}:${minStr}`)}
                    className={`w-full text-center px-2 py-2 text-caption font-mono uppercase font-bold transition-colors ${
                      h === hourStr
...
export function LiveSniffer() {
  ...
  return (
    <div className="font-matrix text-caption text-matrix-primary space-y-1 h-full flex flex-col justify-end">
      {logs.map((log, idx) => (
        <div key={idx} className="shrink-0 animate-pulse border-l-2 border-matrix-ghost pl-2 truncate">
          {log}
        </div>
      ))}
    </div>
...
export function BootScreen({ onSkip, type = "boot" }: BootScreenProps) {
...
`;
  ...
  return (
    <div
      className={`bg-matrix-dark text-matrix-primary font-matrix flex flex-col items-center justify-center p-4 md:p-8 text-center text-body overflow-hidden w-full h-full ${
        canSkip && !isLogout ? "cursor-pointer" : ""
      }`}
      onClick={canSkip && !isLogout ? onSkip : undefined}
      role={canSkip && !isLogout ? "button" : undefined}
      tabIndex={canSkip && !isLogout ? 0 : undefined}
    >
      <div className="w-full max-w-full overflow-hidden flex justify-center mb-6">
        <pre className="text-[4px] min-[360px]:text-[5px] sm:text-[7px] md:text-[10px] lg:text-caption leading-[1.2] text-matrix-muted select-none block text-center">
          {isLogout ? asciiArtLogout : asciiArtBoot}
        </pre>
      </div>
      <div className="animate-pulse tracking-[0.1em] md:tracking-[0.3em] text-[10px] md:text-caption font-bold mb-4 uppercase px-4 text-center break-words max-w-full">
        {isLogout ? t("pages.loading.deleting") : t("pages.loading.initializing")}
      </div>
