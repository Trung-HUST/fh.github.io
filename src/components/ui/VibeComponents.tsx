...
interface IdentityCardProps {
  name?: string;
  avatarUrl?: string;
  avatarConfig?: { name?: string; isAnon?: boolean; isTheOne?: boolean };
  isPublic?: boolean;
  onDecrypt?: () => void;
  title?: string;
  subtitle?: string;
...
export function IdentityCard({
  ...
  // Use avatarConfig if provided, otherwise fallback to default logic
  const finalAvatarName = avatarConfig?.name ?? (isPublic ? name : "unknown");
  const finalIsAnon = avatarConfig?.isAnon ?? !isPublic;
  const finalIsTheOne = avatarConfig?.isTheOne ?? false;
