...
export function TrendMonitor({
  ...
  const formatCompact = (value: number): string => {
    ...
    if (abs >= 1e9) return `${(n / 1e9).toFixed(abs >= 1e10 ? 0 : 1)} Tỷ`;
    if (abs >= 1e6) return `${(n / 1e6).toFixed(abs >= 1e7 ? 0 : 1)} Tr`;
    if (abs >= 1e3) return `${(n / 1e3).toFixed(abs >= 1e4 ? 0 : 1)} N`;
          ...
          <>
            ...
            >
              ...
              {hover.missing ? (
                ...
                <div className="font-bold">{formatFull(hover.value)} {unitLabel}</div>
              )}
            </div>
          </>
        )}
                ...
                {weeks.map((week, wIdx) =>
                  (Array.isArray(week) ? week : []).map((cell, dIdx) => {
                    ...
                    return (
                      <span
                        key={key}
                        title={`${cell.day}: ${formatTokenValue(cellValue)} đ (${tzDetail})`}
