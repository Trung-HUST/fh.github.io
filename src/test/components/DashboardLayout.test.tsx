...
describe("DashboardLayout", () => {
  ...
  describe("sidebar rendering", () => {
    it("renders the MATRIX branding", () => {
      renderLayout();
      expect(screen.getByText("[Dark-Fund]")).toBeInTheDocument();
    });
    ...
    it("renders user footer with name and email", () => {
      ...
      expect(screen.getByText("demo@email.com")).toBeInTheDocument();
    });
  ...
  describe("navigation", () => {
    ...
    it("renders ExternalLink icon for external items", () => {
      renderLayout("/");
      // External items: Static Page, Loading, 404 Page
      // They should not have active styling since they're external
      const staticBtn = screen.getByRole("button", { name: /^Static$/i });
      expect(staticBtn.className).not.toContain("bg-matrix-primary/10");
    });
  });

  describe("logout", () => {
    it("clears cached session and navigates to login", () => {
      window.localStorage.setItem("matrix-auth-user", JSON.stringify({ email: "test@example.com" }));
      window.localStorage.setItem("matrix-sheet-cache", "[]");

      renderLayout();
      fireEvent.click(screen.getByLabelText("Log out"));

      expect(window.localStorage.getItem("matrix-auth-user")).toBeNull();
      expect(window.localStorage.getItem("matrix-sheet-cache")).toBeNull();
      expect(screen.getByTestId("login-page")).toBeInTheDocument();
    });
  });
