...
import { authenticateAndSync, persistSheetSession, clearSheetSession, registerRemoteUser } from "@/lib/googleSheetDb";
import {
  MatrixRain,
  MatrixInput,
  DecodingText,
...
export default function LoginPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      let session;
      if (isRegistering) {
        session = await registerRemoteUser(email, password, displayName);
      } else {
        session = await authenticateAndSync(email, password);
      }
      
      persistSheetSession(session);
      
      if (session.settings && session.settings.language) {
        await i18n.changeLanguage(session.settings.language);
      }

      setIsError(false);
      setMessage(isRegistering ? "Registration successful." : `Synced ${session.transactions.length} records from Google Sheet.`);
      navigate("/");
    } catch (error) {
      setIsError(true);
 ...
 ╚═════╝   ╚═╝   ╚═════╝ ╚═╝`.trim()}</pre>
            ...
            <form className="space-y-3" onSubmit={handleSubmit}>
              <div className="flex items-center gap-2 mb-1">
                ...
                <TypewriterText
                  text={isRegistering ? "INITIALIZE NEW OPERATOR" : t("pages.login.enterCredentials")}
              ...
              {message && (
                <p
                  className={`font-mono text-[10px] uppercase tracking-widest ${
                    isError ? "text-red-400" : "text-matrix-primary"
                  }`}
                >
                  {message}
                </p>
              )}
              <p className="font-mono text-[10px] text-matrix-dim">
                Demo account: <span className="text-matrix-primary">demo@email.com</span>
              </p>

              {/* Actions */}
              <div className="space-y-2">
                <MatrixButton className="w-full" type="submit" loading={isSubmitting}>
                  [ {isRegistering ? "REGISTER" : t("pages.login.authenticate")} ]
                </MatrixButton>
                <div className="flex items-center justify-between mt-2">
                  <button 
                    type="button" 
                    onClick={() => { setIsRegistering(!isRegistering); setMessage(""); setIsError(false); }}
                    className="font-mono text-[10px] text-matrix-primary/80 hover:text-matrix-primary transition-colors underline-offset-4 hover:underline"
                  >
